    ...
    <div class="main_menu">
        <nav class="navbar navbar-expand-lg navbar-light">
            <div class="container box_1620">
                ...
                <a class="navbar-brand logo_h" href="{{ route('theme.homepage') }}"><img
                ...
                <div class="collapse navbar-collapse offset" id="navbarSupportedContent">
                    <ul class="nav navbar-nav menu_nav justify-content-center">
                        <li class="nav-item @yield('active-home')"><a class="nav-link"
                                href="{{ route('theme.homepage') }}">Home</a></li>

                        <li class="nav-item @yield('active-categories') submenu dropdown">
                                ...
                                @if (count($headerCategories) > 0)
                                    @foreach ($headerCategories as $category)
                                        <li class="nav-item "><a class="nav-link"
                                                href="{{ route('theme.categorypage', ['id' => $category->id]) }}">{{ $category->name }}</a>
                                        </li>
                                    @endforeach
                        ...
                        <li class="nav-item @yield('active-contact')"><a class="nav-link"
                                href="{{ route('theme.contactpage') }}">Contact</a></li>
                    </ul>

                    <!-- Add new blog -->
                    @if (Auth::check())
                        <a href="{{ route('blogs.create') }}" class="btn btn-sm btn-primary mr-3">Add Blog</a>
                    @endif
                    <!-- End - Add new blog -->
                    ...
                    <ul class="nav navbar-nav navbar-right navbar-social">
                        ...
                        @else
                            <li class="nav-item submenu dropdown">
                                ...
                                <ul class="dropdown-menu">
                                    <li class="nav-item"><a class="nav-link"
                                            href="{{ route('profile.edit') }}">Dashboard</a></li>
